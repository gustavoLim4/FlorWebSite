...
app.use(cors()); // Adiciona o CORS, permitir apenas o acesso do dominio do seu frontend que está rodando em (localhost:3000)
...
app.post('/api/users/register', (requis, respt) => {
  const { email_cadastros, senha_cadastros } = requis.body;

  // SQL para inserir um novo usuário na tabela 'usuarios'
  const query = 'INSERT INTO cadastros (email_cadastros, senha_cadastros) VALUES (?, ?)';

  // enviando a consulta SQL para o banco de dados. O segundo parâmetro [email, senha]
  db.query(query, [email_cadastros, senha_cadastros], (err) => {
    if (err) {
    ...
    // Retornar sucesso
    respt.status(200).send({ message: "Bem-vindo Cliente"});
  });
});
